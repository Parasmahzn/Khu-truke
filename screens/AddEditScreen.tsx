...
export default function AddEditScreen() {
  ...
  const removeCustomTag = (t: string) => setTags((cur) => cur.filter((x) => x !== t));
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.section}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 6 }}>
            ...
            {customTags.map((t) => (
              <Pressable key={t} style={styles.customTagChip} onPress={() => removeCustomTag(t)}>
                <Text style={styles.customTagText}>#{t}</Text>
                <Text style={styles.customTagX}>×</Text>
              </Pressable>
            ))}
            {addingTag ? (
              <View style={styles.tagInputWrap}>
                ...
                <TextInput
                  ref={tagInputRef}
                  value={tagInput}
                  onChangeText={(v) => setTagInput(v.replace(/[^a-z0-9\-]/gi, '').toLowerCase())}
                  placeholder="tag"
                  placeholderTextColor={C.mute}
              ...
              <Pressable style={styles.addTagChip} onPress={() => setAddingTag(true)}>
...
const makeStyles = (C: Colors) => StyleSheet.create({
  ...
  customTagChip: {
    ...
    borderWidth: 1.5, borderColor: C.purple, backgroundColor: C.purpleSoft,
    marginRight: 6, marginBottom: 6, alignSelf: 'flex-start',
  },
  customTagText: { color: C.purpleDark, fontWeight: '700', fontSize: 12 },
  customTagX: { color: C.purpleDark, fontWeight: '900', fontSize: 14, lineHeight: 16 },
  tagInputWrap: {
    flexDirection: 'row', alignItems: 'center', gap: 2,
    paddingVertical: 4, paddingHorizontal: 10, borderRadius: 999,
    borderWidth: 1.5, borderColor: C.purple, backgroundColor: C.purpleSoft,
    marginRight: 6, marginBottom: 6, minWidth: 90,
  },
  tagHash: { color: C.purpleDark, fontWeight: '700', fontSize: 12 },
  tagInput: { fontSize: 12, color: C.ink, fontWeight: '600', padding: 0, minWidth: 60 },
  addTagChip: {
